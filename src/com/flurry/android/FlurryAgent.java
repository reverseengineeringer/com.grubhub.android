package com.flurry.android;

import android.content.Context;
import android.location.Criteria;
import android.location.Location;
import android.os.Build.VERSION;
import com.flurry.sdk.fu;
import com.flurry.sdk.ha;
import com.flurry.sdk.hn;
import com.flurry.sdk.ho;
import com.flurry.sdk.hp;
import com.flurry.sdk.hw;
import com.flurry.sdk.hx;
import com.flurry.sdk.ib;
import com.flurry.sdk.ja;
import com.flurry.sdk.jb;
import com.flurry.sdk.je;
import com.flurry.sdk.jn;
import com.flurry.sdk.ju;
import java.util.Date;
import java.util.Map;

public final class FlurryAgent
{
  private static final String a = FlurryAgent.class.getSimpleName();
  private static FlurryAgentListener b;
  private static final hw<ja> c = new hw()
  {
    public void a(final ja paramAnonymousja)
    {
      hn.a().a(new Runnable()
      {
        public void run()
        {
          switch (FlurryAgent.2.a[paramAnonymousjac.ordinal()])
          {
          }
          do
          {
            return;
          } while (FlurryAgent.a() == null);
          FlurryAgent.a().onSessionStarted();
        }
      });
    }
  };
  
  public static void addOrigin(String paramString1, String paramString2)
  {
    addOrigin(paramString1, paramString2, null);
  }
  
  public static void addOrigin(String paramString1, String paramString2, Map<String, String> paramMap)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if ((paramString1 == null) || (paramString1.length() == 0)) {
      throw new IllegalArgumentException("originName not specified");
    }
    if ((paramString2 == null) || (paramString2.length() == 0)) {
      throw new IllegalArgumentException("originVersion not specified");
    }
    try
    {
      hp.a().a(paramString1, paramString2, paramMap);
      return;
    }
    catch (Throwable paramString1)
    {
      ib.a(a, "", paramString1);
    }
  }
  
  public static void clearLocation()
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    je.a().a("ExplicitLocation", null);
  }
  
  public static void endTimedEvent(String paramString)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to endTimedEvent was null.");
      return;
    }
    try
    {
      fu.a().b(paramString);
      return;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "Failed to signify the end of event: " + paramString, localThrowable);
    }
  }
  
  public static void endTimedEvent(String paramString, Map<String, String> paramMap)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to endTimedEvent was null.");
      return;
    }
    if (paramMap == null)
    {
      ib.b(a, "String eventId passed to endTimedEvent was null.");
      return;
    }
    try
    {
      fu.a().b(paramString, paramMap);
      return;
    }
    catch (Throwable paramMap)
    {
      ib.a(a, "Failed to signify the end of event: " + paramString, paramMap);
    }
  }
  
  public static int getAgentVersion()
  {
    return ho.a();
  }
  
  public static String getReleaseVersion()
  {
    return ho.f();
  }
  
  public static String getSessionId()
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return null;
    }
    try
    {
      String str = ha.a().c();
      return str;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "", localThrowable);
    }
    return null;
  }
  
  public static void init(Context paramContext, String paramString)
  {
    for (;;)
    {
      try
      {
        if (Build.VERSION.SDK_INT < 10)
        {
          ib.b(a, "Device SDK Version older than 10");
          return;
        }
        if (paramContext == null) {
          throw new NullPointerException("Null context");
        }
      }
      finally {}
      if ((paramString == null) || (paramString.length() == 0)) {
        throw new IllegalArgumentException("apiKey not specified");
      }
      try
      {
        ju.a();
        hn.a(paramContext, paramString);
      }
      catch (Throwable paramContext)
      {
        ib.a(a, "", paramContext);
      }
    }
  }
  
  public static boolean isSessionActive()
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return false;
    }
    try
    {
      boolean bool = jb.a().f();
      return bool;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "", localThrowable);
    }
    return false;
  }
  
  public static FlurryEventRecordStatus logEvent(String paramString)
  {
    FlurryEventRecordStatus localFlurryEventRecordStatus1 = FlurryEventRecordStatus.kFlurryEventFailed;
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return localFlurryEventRecordStatus1;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to logEvent was null.");
      return localFlurryEventRecordStatus1;
    }
    try
    {
      FlurryEventRecordStatus localFlurryEventRecordStatus2 = fu.a().a(paramString);
      return localFlurryEventRecordStatus2;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "Failed to log event: " + paramString, localThrowable);
    }
    return localFlurryEventRecordStatus1;
  }
  
  public static FlurryEventRecordStatus logEvent(String paramString, Map<String, String> paramMap)
  {
    FlurryEventRecordStatus localFlurryEventRecordStatus = FlurryEventRecordStatus.kFlurryEventFailed;
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return localFlurryEventRecordStatus;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to logEvent was null.");
      return localFlurryEventRecordStatus;
    }
    if (paramMap == null)
    {
      ib.b(a, "String parameters passed to logEvent was null.");
      return localFlurryEventRecordStatus;
    }
    try
    {
      paramMap = fu.a().a(paramString, paramMap);
      return paramMap;
    }
    catch (Throwable paramMap)
    {
      ib.a(a, "Failed to log event: " + paramString, paramMap);
    }
    return localFlurryEventRecordStatus;
  }
  
  public static FlurryEventRecordStatus logEvent(String paramString, Map<String, String> paramMap, boolean paramBoolean)
  {
    FlurryEventRecordStatus localFlurryEventRecordStatus = FlurryEventRecordStatus.kFlurryEventFailed;
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return localFlurryEventRecordStatus;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to logEvent was null.");
      return localFlurryEventRecordStatus;
    }
    if (paramMap == null)
    {
      ib.b(a, "String parameters passed to logEvent was null.");
      return localFlurryEventRecordStatus;
    }
    try
    {
      paramMap = fu.a().a(paramString, paramMap, paramBoolean);
      return paramMap;
    }
    catch (Throwable paramMap)
    {
      ib.a(a, "Failed to log event: " + paramString, paramMap);
    }
    return localFlurryEventRecordStatus;
  }
  
  public static FlurryEventRecordStatus logEvent(String paramString, boolean paramBoolean)
  {
    FlurryEventRecordStatus localFlurryEventRecordStatus1 = FlurryEventRecordStatus.kFlurryEventFailed;
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return localFlurryEventRecordStatus1;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to logEvent was null.");
      return localFlurryEventRecordStatus1;
    }
    try
    {
      FlurryEventRecordStatus localFlurryEventRecordStatus2 = fu.a().a(paramString, paramBoolean);
      return localFlurryEventRecordStatus2;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "Failed to log event: " + paramString, localThrowable);
    }
    return localFlurryEventRecordStatus1;
  }
  
  public static void onEndSession(Context paramContext)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramContext == null) {
      throw new NullPointerException("Null context");
    }
    if (hn.a() == null) {
      throw new IllegalStateException("Flurry SDK must be initialized before ending a session");
    }
    try
    {
      jb.a().c(paramContext);
      return;
    }
    catch (Throwable paramContext)
    {
      ib.a(a, "", paramContext);
    }
  }
  
  @Deprecated
  public static void onError(String paramString1, String paramString2, String paramString3)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString1 == null)
    {
      ib.b(a, "String errorId passed to onError was null.");
      return;
    }
    if (paramString2 == null)
    {
      ib.b(a, "String message passed to onError was null.");
      return;
    }
    if (paramString3 == null)
    {
      ib.b(a, "String errorClass passed to onError was null.");
      return;
    }
    try
    {
      fu.a().a(paramString1, paramString2, paramString3);
      return;
    }
    catch (Throwable paramString1)
    {
      ib.a(a, "", paramString1);
    }
  }
  
  public static void onError(String paramString1, String paramString2, Throwable paramThrowable)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString1 == null)
    {
      ib.b(a, "String errorId passed to onError was null.");
      return;
    }
    if (paramString2 == null)
    {
      ib.b(a, "String message passed to onError was null.");
      return;
    }
    if (paramThrowable == null)
    {
      ib.b(a, "Throwable passed to onError was null.");
      return;
    }
    try
    {
      fu.a().a(paramString1, paramString2, paramThrowable);
      return;
    }
    catch (Throwable paramString1)
    {
      ib.a(a, "", paramString1);
    }
  }
  
  @Deprecated
  public static void onEvent(String paramString)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to onEvent was null.");
      return;
    }
    try
    {
      fu.a().c(paramString);
      return;
    }
    catch (Throwable paramString)
    {
      ib.a(a, "", paramString);
    }
  }
  
  @Deprecated
  public static void onEvent(String paramString, Map<String, String> paramMap)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String eventId passed to onEvent was null.");
      return;
    }
    if (paramMap == null)
    {
      ib.b(a, "Parameters Map passed to onEvent was null.");
      return;
    }
    try
    {
      fu.a().c(paramString, paramMap);
      return;
    }
    catch (Throwable paramString)
    {
      ib.a(a, "", paramString);
    }
  }
  
  public static void onPageView()
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    try
    {
      fu.a().g();
      return;
    }
    catch (Throwable localThrowable)
    {
      ib.a(a, "", localThrowable);
    }
  }
  
  public static void onStartSession(Context paramContext)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramContext == null) {
      throw new NullPointerException("Null context");
    }
    if (hn.a() == null) {
      throw new IllegalStateException("Flurry SDK must be initialized before starting a session");
    }
    try
    {
      jb.a().b(paramContext);
      return;
    }
    catch (Throwable paramContext)
    {
      ib.a(a, "", paramContext);
    }
  }
  
  @Deprecated
  public static void onStartSession(Context paramContext, String paramString)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramContext == null) {
      throw new NullPointerException("Null context");
    }
    if ((paramString == null) || (paramString.length() == 0)) {
      throw new IllegalArgumentException("Api key not specified");
    }
    if (hn.a() == null) {
      throw new IllegalStateException("Flurry SDK must be initialized before starting a session");
    }
    try
    {
      jb.a().b(paramContext);
      return;
    }
    catch (Throwable paramContext)
    {
      ib.a(a, "", paramContext);
    }
  }
  
  public static void setAge(int paramInt)
  {
    if (Build.VERSION.SDK_INT < 10) {
      ib.b(a, "Device SDK Version older than 10");
    }
    while ((paramInt <= 0) || (paramInt >= 110)) {
      return;
    }
    long l = new Date(new Date(System.currentTimeMillis() - paramInt * 31449600000L).getYear(), 1, 1).getTime();
    je.a().a("Age", Long.valueOf(l));
  }
  
  public static void setCaptureUncaughtExceptions(boolean paramBoolean)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    je.a().a("CaptureUncaughtExceptions", Boolean.valueOf(paramBoolean));
  }
  
  public static void setContinueSessionMillis(long paramLong)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramLong < 5000L)
    {
      ib.b(a, "Invalid time set for session resumption: " + paramLong);
      return;
    }
    je.a().a("ContinueSessionMillis", Long.valueOf(paramLong));
  }
  
  public static void setFlurryAgentListener(FlurryAgentListener paramFlurryAgentListener)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramFlurryAgentListener == null)
    {
      ib.b(a, "Listener cannot be null");
      hx.a().b("com.flurry.android.sdk.FlurrySessionEvent", c);
      return;
    }
    b = paramFlurryAgentListener;
    hx.a().a("com.flurry.android.sdk.FlurrySessionEvent", c);
  }
  
  public static void setGender(byte paramByte)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    switch (paramByte)
    {
    default: 
      je.a().a("Gender", Byte.valueOf((byte)-1));
      return;
    }
    je.a().a("Gender", Byte.valueOf(paramByte));
  }
  
  public static void setLocation(float paramFloat1, float paramFloat2)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    Location localLocation = new Location("Explicit");
    localLocation.setLatitude(paramFloat1);
    localLocation.setLongitude(paramFloat2);
    je.a().a("ExplicitLocation", localLocation);
  }
  
  @Deprecated
  public static void setLocationCriteria(Criteria paramCriteria)
  {
    if (Build.VERSION.SDK_INT < 10) {
      ib.b(a, "Device SDK Version older than 10");
    }
  }
  
  public static void setLogEnabled(boolean paramBoolean)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramBoolean)
    {
      ib.b();
      return;
    }
    ib.a();
  }
  
  public static void setLogEvents(boolean paramBoolean)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    je.a().a("LogEvents", Boolean.valueOf(paramBoolean));
  }
  
  public static void setLogLevel(int paramInt)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    ib.a(paramInt);
  }
  
  public static void setPulseEnabled(boolean paramBoolean)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    je.a().a("ProtonEnabled", Boolean.valueOf(paramBoolean));
  }
  
  public static void setReportLocation(boolean paramBoolean)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    je.a().a("ReportLocation", Boolean.valueOf(paramBoolean));
  }
  
  public static void setUserId(String paramString)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String userId passed to setUserId was null.");
      return;
    }
    je.a().a("UserId", jn.b(paramString));
  }
  
  public static void setVersionName(String paramString)
  {
    if (Build.VERSION.SDK_INT < 10)
    {
      ib.b(a, "Device SDK Version older than 10");
      return;
    }
    if (paramString == null)
    {
      ib.b(a, "String versionName passed to setVersionName was null.");
      return;
    }
    je.a().a("VersionName", paramString);
  }
}

/* Location:
 * Qualified Name:     com.flurry.android.FlurryAgent
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */