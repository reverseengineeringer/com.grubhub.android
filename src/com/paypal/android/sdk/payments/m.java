package com.paypal.android.sdk.payments;

import android.content.Context;
import android.view.View;
import android.webkit.WebView;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ScrollView;
import android.widget.TextView;
import com.paypal.android.sdk.eb;
import com.paypal.android.sdk.ec;
import com.paypal.android.sdk.ek;
import com.paypal.android.sdk.em;
import com.paypal.android.sdk.ft;
import java.util.Arrays;
import java.util.HashSet;
import java.util.Locale;
import java.util.Set;

final class m
{
  View a;
  TextView b;
  Button c;
  private ScrollView d;
  private LinearLayout e;
  private ImageView f;
  
  m(Context paramContext, l paraml)
  {
    d = new ScrollView(paramContext);
    d.setBackgroundColor(eb.d);
    e = new LinearLayout(paramContext);
    e.setOrientation(1);
    d.addView(e);
    ec.a(e, -1, -2);
    LinearLayout localLinearLayout = new LinearLayout(paramContext);
    localLinearLayout.setOrientation(1);
    ec.a(localLinearLayout, "16dip", "20dip", "16dip", "20dip");
    e.addView(localLinearLayout);
    ec.a(localLinearLayout, -1, -2);
    f = new ImageView(paramContext);
    f.setImageBitmap(ec.b("", paramContext));
    f.setContentDescription("PayPal");
    localLinearLayout.addView(f);
    ec.a(f, -1, "40dip");
    ec.b(f, null, null, null, "10dip");
    b = new TextView(paramContext);
    WebView localWebView = new WebView(paramContext);
    localWebView.setBackgroundColor(eb.d);
    if (ek.a) {}
    for (String str1 = "direction: rtl;";; str1 = "")
    {
      str1 = "body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; " + str1 + "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }" + "h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }" + "ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }";
      StringBuilder localStringBuilder = new StringBuilder("<html><meta name='viewport' content='width=device-width; initial-scale=1.0;'/><head><style type=\"text/css\">").append(str1).append("</style></head><body>");
      switch (n.a[paraml.ordinal()])
      {
      default: 
        paraml = null;
        localWebView.loadData(paraml + "</body><html>", "text/html; charset=UTF-8", null);
        localWebView.setClickable(true);
        localLinearLayout.addView(localWebView);
        ec.a(localWebView, -1, -2);
        ec.b(localWebView, null, null, null, "10dip");
        c = new Button(paramContext);
        localLinearLayout.addView(c);
        ec.a(c, true, paramContext);
        ec.b(c, null, "20dip", null, "4dip");
        a = d;
        return;
      }
    }
    String str2 = ek.a(em.am);
    str1 = Locale.getDefault().getCountry().toLowerCase(Locale.US);
    paraml = str1;
    if (ft.b(str1)) {
      paraml = "us";
    }
    if (new HashSet(Arrays.asList(new String[] { "nl", "de", "it", "fr", "gb", "us" })).contains(paraml)) {}
    for (paraml = String.format("https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full", new Object[] { paraml });; paraml = "")
    {
      paraml = String.format(str2, new Object[] { paraml });
      break;
      paraml = ek.a(em.ai);
      break;
      paraml = String.format(ek.a(em.aH), new Object[] { ek.a(em.aG) });
      break;
      paraml = String.format(ek.a(em.aH), new Object[] { ek.a(em.aJ) });
      break;
    }
  }
}

/* Location:
 * Qualified Name:     com.paypal.android.sdk.payments.m
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */